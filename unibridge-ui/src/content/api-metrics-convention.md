...
path)로 등록하면 30초 내에 수집이 시작됩니다. 기본은 **HTTP**로 수집하며,
HTTPS로만 열린 서비스는 등록 시 프로토콜을 `https`로 선택하세요 — 사내
자체서명 인증서도 동작합니다(수집 시 인증서 검증을 생략). 등록/수집
파이프라인은 서버 모니터링과 같은 구조를 사용합니다 —
[server-monitoring.md](server-monitoring.md) 참고.
